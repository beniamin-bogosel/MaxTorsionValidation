#!/usr/bin/env python3
"""Certify polygons using rotation-cached candidates and unchanged C verifiers.
...
def main():
    ...
    sources.mkdir()
    paths = [Path(__file__).resolve(), ROOT/'MaxTorsionValidation/freefem/torsion_hessian_rotations.edp',
             ROOT/'MaxTorsionValidation/freefem/second_variation_lifting_rotations.edp',
             ROOT/'MaxTorsionValidation/flint/second_lifting_cert.c', ROOT/'MaxTorsionValidation/flint/mode_cert.c',
             ROOT/'MaxTorsionValidation/flint/Makefile', ROOT/'MaxTorsionValidation/freefem/rotation_cache.py',
             ROOT/'MaxTorsionValidation/freefem/certificate_build.py']
    for path in paths:
        shutil.copy2(path, sources/path.name)
    ff = os.environ.get('FREEFEM_BIN', 'FreeFem++')
    manifest = dict(candidate_scheme='first_vertex_rotations', first=args.first, last=args.last, m=args.m, precision=192,
    ...
    for n in range(args.first, args.last+1):
        ...
        if not args.entry_centers:
            run([ff, '-v', '0', '-nw', ROOT/'MaxTorsionValidation/freefem/torsion_hessian_rotations.edp',
                 '-n', n, '-m', args.m], modes)
        ...
        from rotation_cache import prepare
        basis = folder/'first_vertex_basis.txt'
        run([ff, '-v', '0', '-nw', ROOT/'MaxTorsionValidation/freefem/second_variation_lifting_rotations.edp',
             '-n', n, '-m', args.m, '-k', 1, '-qa', 0, '-ra', 1,
             '-make-basis', 1, '-flux-eps', args.flux_eps,
             '-export-second', basis], folder/'first_vertex_basis.log')
        cache_folder = folder/'rotation_cache'
        cache_result = prepare(basis, cache_folder)
        (folder/'rotation_cache.json').write_text(json.dumps(cache_result, indent=2)+'\n')
        with basis.open('rb') as src, gzip.open(str(basis)+'.gz', 'wb', compresslevel=6) as dst:
            shutil.copyfileobj(src, dst)
        basis.unlink()
        print(f'ROTATION_CACHE_READY n={n} pde_solves=3 elapsed={time.monotonic()-start:.1f}s', flush=True)
        ...
        def evaluate(spec):
            ...
            run([ff, '-v', '0', '-nw', ROOT/'MaxTorsionValidation/freefem/second_variation_lifting_rotations.edp',
                 ...
                 '-cached-fields', cache_folder/f'{tag}.txt',
